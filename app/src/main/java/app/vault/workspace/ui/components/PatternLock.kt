...
fun PatternLock(
    ...
    val selected = remember(resetKey) { mutableStateListOf<Int>() }
    var finger by remember(resetKey) { mutableStateOf<Offset?>(null) }
            ...
            modifier = Modifier
                ...
                .pointerInput(enabled, resetKey) {
                    ...
                    awaitEachGesture {
                        ...
                        hitTest(down.position)?.let { addCell(it) }
                        drag(down.id) { change ->
                            change.consume()
                            finger = change.position
                            hitTest(change.position)?.let { addCell(it) }
                        }
                        finish()
                    }
                },
